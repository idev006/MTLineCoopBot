...

{
  const d=makeDeps({
    members:[
      ...
      {mem_code:'M4',mem_status:'active',mem_role:'member',line_user_id:'U4',mem_eff_dt:'2026-01-01',mem_exp_dt:'2026-12-31'}
    ]
  });
  ...
  const s=uc.execute();
  if(s.checked!==4 || s.logged!==3 || s.expiring!==1 || s.expired!==1 || s.pushed!==2) throw new Error('expiry summary mismatch');
  if(d.messaging.list().length!==2) throw new Error('expiry messages mismatch');
  if(d.messaging.list().some(m=>m.to==='U4')) throw new Error('valid active member must not receive expiry push');
  if(d.menu.listRevoked()[0]!=='U2' || d.menu.listRevoked().includes('U1')) throw new Error('only expired member menu must be revoked');
  if(d.audit.snapshot().length!==3) throw new Error('expiry audit mismatch');
}

{
  ...
  const again=uc.execute();
  if(again.pending!==0 || again.pushed!==0 || d.messaging.list().length!==1) throw new Error('sent notice must not broadcast twice');
}

{
  const d=makeDeps({
    ...
    loans:[
      ...
      {mem_code:'M1',loan_no:'L4',due_dt:'2026-09-01'}
    ]
  });
  ...
  const s=uc.execute();
  if(s.loans!==4 || s.due!==2 || s.reminded!==1 || s.skipped!==1 || s.pushed!==1) throw new Error('loan reminder summary mismatch');
